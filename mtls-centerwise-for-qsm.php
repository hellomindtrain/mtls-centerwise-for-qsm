...
function mtls_qsm_centerwise_activate_plugin() {
    global $wpdb;
    ...
    // Updated prefixes for DB tables
    // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    $sql_centers = "CREATE TABLE {$wpdb->prefix}mtls_qsm_centers (id mediumint(9) NOT NULL AUTO_INCREMENT, custom_center_id varchar(50) NOT NULL, center_name varchar(255) NOT NULL, manager_id bigint(20) NOT NULL, city varchar(100) DEFAULT '' NOT NULL, address text, status varchar(20) DEFAULT 'active' NOT NULL, created_at datetime DEFAULT CURRENT_TIMESTAMP NOT NULL, PRIMARY KEY (id), UNIQUE KEY custom_center_id (custom_center_id)) {$charset};";
    dbDelta($sql_centers);
    
    // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared, WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    $sql_students = "CREATE TABLE {$wpdb->prefix}mtls_qsm_students (id mediumint(9) NOT NULL AUTO_INCREMENT, user_id bigint(20) NOT NULL, custom_center_id varchar(50) NOT NULL, student_name varchar(255) NOT NULL, phone_number varchar(20) NOT NULL, address text NOT NULL, city varchar(100) DEFAULT '' NOT NULL, course_name varchar(100) NOT NULL, admission_date date NOT NULL, status varchar(20) DEFAULT 'active' NOT NULL, PRIMARY KEY (id)) {$charset};";
...
function mtls_qsm_get_manager_custom_center_id($user_id) {
    global $wpdb;
...
// 2. DIRECT ASSETS INJECTION (FOOLPROOF CSS/JS)
function mtls_qsm_print_inline_assets() {
    ...
    <script>
    function mtlsQsmToggleCenterData(id) {
        var el = document.getElementById('mtls-qsm-center-data-' + id);
...
add_action( 'init', 'mtls_qsm_handle_all_forms' );
function mtls_qsm_handle_all_forms() {
    ...
    if ( isset($_POST['mtls_delete_center'], $_POST['mtls_del_center_nonce'], $_POST['delete_center_id']) && current_user_can('manage_options') ) {
        if( wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['mtls_del_center_nonce'])), 'mtls_del_center_action') ) {
            ...
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $mgr_id = $wpdb->get_var($wpdb->prepare("SELECT manager_id FROM {$wpdb->prefix}mtls_qsm_centers WHERE custom_center_id = %s", $del_center_id));
            if ($mgr_id) {
                if ( ! function_exists( 'wp_delete_user' ) ) { require_once ABSPATH . 'wp-admin/includes/user.php'; }
                wp_delete_user($mgr_id);
            }
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $orphan_students = $wpdb->get_col($wpdb->prepare("SELECT user_id FROM {$wpdb->prefix}mtls_qsm_students WHERE custom_center_id = %s", $del_center_id));
            if (!empty($orphan_students)) {
                if ( ! function_exists( 'wp_delete_user' ) ) { require_once ABSPATH . 'wp-admin/includes/user.php'; }
                foreach ($orphan_students as $os_id) { wp_delete_user($os_id); }
                // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                $wpdb->delete("{$wpdb->prefix}mtls_qsm_students", ['custom_center_id' => $del_center_id]);
            }
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $wpdb->delete("{$wpdb->prefix}mtls_qsm_centers", ['custom_center_id' => $del_center_id]);
            $mtls_qsm_sa_msg = '<div style="color:#166534; background:#dcfce7; padding:12px; border-radius:6px; margin-bottom:20px; font-weight:bold;">Center removed successfully.</div>';
        }
    }
    ...
    if ( isset($_POST['mtls_sa_reg_submit'], $_POST['mtls_sa_reg_nonce']) && current_user_can('manage_options') ) {
        if( wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['mtls_sa_reg_nonce'])), 'mtls_sa_reg_action') ) {
            ...
            if ($password !== $confirm_pass) {
                $mtls_qsm_sa_msg = '<div style="color:#ef4444; background:#fee2e2; padding:12px; border-radius:6px; margin-bottom:20px; font-weight:bold;">Error: Passwords do not match.</div>';
            } elseif ( username_exists($username) || email_exists($email) ) { 
                $mtls_qsm_sa_msg = '<div style="color:#ef4444; background:#fee2e2; padding:12px; border-radius:6px; margin-bottom:20px; font-weight:bold;">Error: Username or Email already taken.</div>'; 
            } else {
                $user_id = wp_create_user( $username, $password, $email );
                if ( ! is_wp_error($user_id) ) {
                    wp_update_user(array('ID' => $user_id, 'role' => 'center_owner'));
                    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                    $wpdb->insert($wpdb->prefix . 'mtls_qsm_centers', array('custom_center_id' => $custom_center_id, 'center_name' => $center_name, 'manager_id' => $user_id, 'address' => $address, 'city' => $city));
                    $mtls_qsm_sa_msg = '<div style="color:#166534; background:#dcfce7; padding:12px; border-radius:6px; margin-bottom:20px; font-weight:bold;">Success! New Center successfully registered.</div>';
                }
            }
        }
    }
    ...
    if ( isset($_POST['mtls_delete_student'], $_POST['mtls_del_stud_nonce'], $_POST['delete_student_id']) && is_user_logged_in() ) {
        if( wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['mtls_del_stud_nonce'])), 'mtls_del_stud_action') ) {
            ...
            $assigned_custom_id = mtls_qsm_get_manager_custom_center_id($current_user_id);
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $verify = $wpdb->get_var($wpdb->prepare("SELECT id FROM {$wpdb->prefix}mtls_qsm_students WHERE user_id = %d AND custom_center_id = %s", $del_id, $assigned_custom_id));
            if ($verify) {
                if ( ! function_exists( 'wp_delete_user' ) ) { require_once ABSPATH . 'wp-admin/includes/user.php'; }
                wp_delete_user($del_id);
                // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                $wpdb->delete("{$wpdb->prefix}mtls_qsm_students", ['user_id' => $del_id]);
                $mtls_qsm_student_msg = '<div style="color:#166534; padding:15px; background:#dcfce7; border-radius:8px; margin-bottom:20px; font-weight:bold;">Student removed successfully.</div>';
            }
        }
    }
    ...
    if ( isset($_POST['mtls_front_add_student'], $_POST['mtls_add_stud_nonce']) && is_user_logged_in() ) {
        if( wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['mtls_add_stud_nonce'])), 'mtls_add_stud_action') ) {
            ...
            $admission_date = !empty($_POST['admission_date']) ? sanitize_text_field(wp_unslash($_POST['admission_date'])) : current_time('Y-m-d');
            
            $assigned_custom_id = mtls_qsm_get_manager_custom_center_id(get_current_user_id());
            ...
            if ($stud_password !== $confirm_pass) {
                $mtls_qsm_student_msg = '<div style="color:#ef4444; background:#fee2e2; padding:10px; border-radius:6px; margin-bottom:15px;">Error: Passwords do not match.</div>';
            } elseif ( username_exists($stud_username) || (!empty($_POST['stud_email']) && email_exists($stud_email)) ) {
                $mtls_qsm_student_msg = '<div style="color:#ef4444; background:#fee2e2; padding:10px; border-radius:6px; margin-bottom:15px;">Error: Username or Email already exists.</div>';
            } else {
                ...
                if ( ! is_wp_error($student_user_id) ) {
                    ...
                    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
                    $wpdb->insert( "{$wpdb->prefix}mtls_qsm_students", array(
                    ...
                    $mtls_qsm_student_msg = '<div style="color:#166534; padding:15px; background:#dcfce7; border-radius:8px; margin-bottom:20px; font-weight:bold;">Student Enrolled Successfully!</div>';
                } else {
                    $mtls_qsm_student_msg = '<div style="color:#ef4444; background:#fee2e2; padding:10px; border-radius:6px; margin-bottom:15px;">Error: ' . esc_html($student_user_id->get_error_message()) . '</div>';
                }
            }
        }
    }
...
add_action('admin_menu', 'mtls_qsm_centerwise_admin_menu');
function mtls_qsm_centerwise_admin_menu() {
    add_menu_page('MTLS Centerwise', 'MTLS Centerwise', 'manage_options', 'mtls-centerwise', 'mtls_qsm_centerwise_admin_page', 'dashicons-bank', 99);
}

function mtls_qsm_centerwise_admin_page() {
...
function mtls_qsm_render_super_admin_dashboard() {
    ...
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    $total_centers_count = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}mtls_qsm_centers");
    
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
    $centers = $wpdb->get_results("SELECT c.*, u.user_email FROM {$wpdb->prefix}mtls_qsm_centers c LEFT JOIN {$wpdb->prefix}users u ON c.manager_id = u.ID");
    ...
    <div class="sa-box">
        ...
        <div class="sa-body">
            ...
            <div style="background: linear-gradient(135deg, #4f46e5, #ec4899); padding: 35px; border-radius: 12px; color: white; margin-bottom: 40px; box-shadow: 0 10px 30px rgba(79, 70, 229, 0.3);">
                <h2 style="color: white; margin-top: 0; font-size: 26px; font-weight: 900;">Unlock MTLS Centerwise PRO (For QSM)</h2>
                ...
                <a href="<?php echo esc_url(MTLS_QSM_PRO_LINK); ?>" target="_blank" style="display: inline-block; padding: 14px 28px; background: #FFD700; color: #1e1b4b; text-decoration: none; font-weight: 800; border-radius: 8px; font-size:15px; box-shadow: 0 4px 15px rgba(0,0,0,0.2); transition: 0.3s;">Explore PRO Features &rarr;</a>
            </div>
            ...
            <div style="background:#fff; border:1px solid #e2e8f0; padding:30px; border-radius:12px; box-shadow:0 4px 15px rgba(0,0,0,0.02); margin-bottom:40px;">
                ...
                <?php echo wp_kses_post($mtls_qsm_sa_msg); ?>
            ...
            <ul class="sa-center-list">
                ...
                if($centers) {
                    foreach($centers as $c) {
                        ...
                        echo '<div id="mtls-qsm-center-data-'.esc_attr($c->custom_center_id).'" class="sa-content-area">';
                        ...
                        if($c_results) {
                            ...
                            foreach($c_results as $res) {
                                ...
                                $date = gmdate("d M Y h:i A", strtotime($res->time_taken));
                                $score = isset($res->correct_score) ? $res->correct_score : '0';

                                echo "<tr><td style='color:#64748b;'>".esc_html($date)."</td><td><strong>".esc_html($res->student_name)."</strong></td><td style='color:#6366f1;'>".esc_html($quiz_name)."</td><td><span class='sa-badge'>".esc_html($score)."%</span></td></tr>";
                            }
                            echo '</tbody></table>';
                        } else {
...
// 6. MAIN FRONTEND DASHBOARD SHORTCODE [mtls_qsm_dashboard]
...
function mtls_qsm_frontend_portal_shortcode() {
    ...
    <div class="mtls-portal-box">
        ...
        if ( $is_student_logged ): 
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $student_profile = $wpdb->get_row( $wpdb->prepare("SELECT s.*, c.center_name FROM {$wpdb->prefix}mtls_qsm_students s JOIN {$wpdb->prefix}mtls_qsm_centers c ON s.custom_center_id = c.custom_center_id WHERE s.user_id = %d", $current_user->ID) );
        ?>
                ...
                <?php elseif ( $active_tab === 'tests' ): ?>
                    ...
                    if ( $results ) {
                        ...
                        foreach($results as $result) {
                            ...
                            $date = gmdate("d M Y h:i A", strtotime($result->time_taken));
                            $score = isset($result->correct_score) ? $result->correct_score : '0';
        ...
        // ==========================================
            ...
            $assigned_custom_id = mtls_qsm_get_manager_custom_center_id($current_user->ID);
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $total_students = (int) $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$wpdb->prefix}mtls_qsm_students WHERE custom_center_id = %s", $assigned_custom_id));
            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
            $center_details = $wpdb->get_row($wpdb->prepare("SELECT * FROM {$wpdb->prefix}mtls_qsm_centers WHERE custom_center_id = %s", $assigned_custom_id));
        ?>
            <ul class="mtls-portal-tabs">
                ...
                <li class="mtls-tab-item <?php echo $active_tab === 'qsm_results' ? 'active' : ''; ?>"><a href="<?php echo esc_url(add_query_arg('tab', 'qsm_results', $current_page_url)); ?>">Exam Results</a></li>
            </ul>
            <div class="mtls-portal-body">
                <?php if ( $active_tab === 'profile' ): ?>
                ...
                <?php elseif ( $active_tab === 'students' ): ?>
                    ...
                    <?php echo wp_kses_post($mtls_qsm_student_msg); ?>
                    <div style="display:flex; flex-direction:column; gap:35px;">
                       ...
                       <div style="width:100%;">
                            ...
                            <?php 
                            // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching, WordPress.DB.PreparedSQL.NotPrepared
                            $student_records = $wpdb->get_results($wpdb->prepare("SELECT s.*, u.user_login FROM {$wpdb->prefix}mtls_qsm_students s LEFT JOIN {$wpdb->prefix}users u ON s.user_id = u.ID WHERE s.custom_center_id = %s ORDER BY s.id DESC", $assigned_custom_id));
                            if($student_records) {
                ...
                <?php elseif ( $active_tab === 'qsm_results' ): ?>
...
add_action( 'delete_user', 'mtls_qsm_centerwise_sync_backend_delete' );
function mtls_qsm_centerwise_sync_backend_delete( $user_id ) {
    global $wpdb;
    
    // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    $center_id = $wpdb->get_var($wpdb->prepare("SELECT custom_center_id FROM {$wpdb->prefix}mtls_qsm_centers WHERE manager_id = %d", $user_id));
    
    if ( !empty($center_id) ) {
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $wpdb->delete("{$wpdb->prefix}mtls_qsm_students", ['custom_center_id' => $center_id]);
        
        // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
        $wpdb->delete("{$wpdb->prefix}mtls_qsm_centers", ['manager_id' => $user_id]);
    }
